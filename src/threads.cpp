...
extern "C" SEXP c_Rth_get_num_threads()
{
  ...
  #if RTH_OMP
  if (rth_nthreads != NULL) INT(nthreads) = atoi(rth_nthreads);
  else if (omp_nthreads != NULL) INT(nthreads) = atoi(omp_nthreads);
  else INT(nthreads) = omp_get_max_threads();
  #elif RTH_TBB
  if (rth_nthreads != NULL) INT(nthreads) = atoi(rth_nthreads);
  else INT(nthreads) = tbb::task_scheduler_init::automatic;
  #elif RTH_CUDA
    INT(nthreads) = 1;
  #else
  ...
  if (INT(nthreads) <= 0) INT(nthreads) = 1;
